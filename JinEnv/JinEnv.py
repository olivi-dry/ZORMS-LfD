...
class SinglePendulum:
    ...
    def initConstraints(self, ):
        self.final_equ = (self.X[0] - pi) ** 2

        max_u = SX.sym('max_u')
        path_inequ_1 = self.U - max_u
        path_inequ_2 = -self.U - max_u

        self.path_inequ = vcat([path_inequ_1, path_inequ_2])
        self.constraint_auxvar = vcat([max_u])
...
class RobotArm:
    ...
    def initCost_Quadratic(self, wu=0.1):
        goal_X = [pi / 2, 0, 0, 0]
        path_cost_diag_A = SX.sym('path_cost_diag_A', self.X.numel())
        path_cost_b = SX.sym('path_cost_b', self.X.numel())
        self.path_cost = (self.X - goal_X).T @ diag(path_cost_diag_A) @ (self.X - goal_X) + wu * dot(self.U, self.U)
        self.final_cost = (self.X - goal_X).T @ diag(path_cost_diag_A) @ (self.X - goal_X)
        self.cost_auxvar = vcat([path_cost_diag_A])

    def initConstraints(self, max_u=None, max_q=None):
        # # # set final constraint h_final(x)=0 and g_final(x)<=0
        # goal_X = [pi / 2, 0, 0, 0]
        # self.final_inequ = dot((self.X[0:2] - goal_X[0:2]), (self.X[0:2] - goal_X[0:2]))
        ...
        if max_q is None:
            max_q = SX.sym('max_q')
            constraint_auxvar += [max_q]
        self.constraint_auxvar = vcat(constraint_auxvar)

        path_inequ_1 = self.U - max_u
        path_inequ_2 = -self.U - max_u

        path_inequ_3 = self.X[0:2] - max_q
        path_inequ_4 = -self.X[0:2] - max_q

        self.path_inequ = vcat([path_inequ_1, path_inequ_2, path_inequ_3, path_inequ_4])
        self.constraint_auxvar = vcat([max_u, max_q])

    def initConstraints2(self, max_dq1, max_dq2):
        path_inequ_1 = self.X[2] - max_dq1
        path_inequ_2 = -self.X[2] - max_dq1
        path_inequ_3 = self.X[3] - max_dq2
        path_inequ_4 = -self.X[3] - max_dq2

        self.constraint_auxvar = []

        self.path_inequ = vcat([path_inequ_1, path_inequ_2, path_inequ_3, path_inequ_4])

    def play_animation(self, l1, l2, dt, state_traj, state_traj_ref=None, save_option=0, title='Robot arm vertical reaching'):
        ...
        # set lines
        line, = ax.plot([], [], 'o-', lw=3)
        line_ref, = ax.plot([], [], color='gray', marker='o', lw=2, alpha=0.7)
        ax.plot(0, 2, 'r^')
        ...
        if save_option != 0:
            ...
            ani.save('./videos/'+title + '.mp4', writer=writer)
            print('save_success')
...
class Quadrotor:
    ...
    def initConstraints(self, max_u=None, max_r=None):
        # set path constraint h_final(x)
        constraint_auxvar = []
        if max_u is None:
            max_u = SX.sym('max_u')
        ...
        if max_r is None:
            max_r = SX.sym('max_x')
            constraint_auxvar += [max_r]
        self.constraint_auxvar = vcat(constraint_auxvar)

        path_inequ_Uub = self.U - max_u
        path_inequ_Ulb = -self.U - max_u
        path_inequ_Xub = dot(self.X[0:3], self.X[0:3]) - max_r
        self.path_inequ = vcat([path_inequ_Uub, path_inequ_Ulb, path_inequ_Xub])
    ...
    def play_animation(self, wing_len, state_traj, state_traj_ref=None, dt=0.1, save_option=0, title='Quadrotor maneuvering'):
        fig = plt.figure()
...
class Rocket:
    ...
    def initDyn(self, Jx=None, Jy=None, Jz=None, mass=None, l=None):
        ...
        dw = mtimes(inv(self.J_B),
                    mtimes(skew(self.r_T_B), self.T_B) -
                    mtimes(mtimes(skew(self.w_B), self.J_B), self.w_B))
    ...
    def initConstraints(self, max_f_sq=None, max_tilt_angle=None):
        # set path constraint h_final(x)
        constraint_auxvar = []
        if max_f_sq is None:
            max_f_sq = SX.sym('max_f_sq')
            constraint_auxvar += [max_f_sq]
        if max_tilt_angle is None:
            max_tilt_angle = SX.sym('max_tilt_angle')
            constraint_auxvar += [max_tilt_angle]
        self.constraint_auxvar = vcat(constraint_auxvar)

        # control bounded
        path_inequ_Uub = dot(self.U, self.U) - max_f_sq

        # title angle bounded
        q = self.X[6:10]
        R_B_I = self.dir_cosine(q)
        goal_q = toQuaternion(0, [1, 0, 0])
        goal_R_B_I = self.dir_cosine(goal_q)
        title_cos = (trace(mtimes(transpose(goal_R_B_I), R_B_I)) - 1) / 2
        path_title_ub = cos(max_tilt_angle) - title_cos

        self.path_inequ = vcat([path_inequ_Uub, path_title_ub])

    def getTiltAngle(self, state_traj):

        # earth upward direction
        goal_q = toQuaternion(0, [1, 0, 0])
        goal_R_B_I = self.dir_cosine(goal_q)
        ...
        for state in state_traj:
            ...
            R_B_I = self.dir_cosine(q)
            title_cos = (trace(mtimes(transpose(goal_R_B_I), R_B_I)) - 1) / 2
            title_angle += [arccos(title_cos)]

        return np.array(title_angle)
