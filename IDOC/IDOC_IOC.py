...
from IDOC import IDOC_eq as idoc_eq
from IDOC import IDOC_ineq as idoc_ineq
...
class IDOC:
    def __init__(self, OC_sys, reference_traj, init_params, learning_rate, num_iter, gamma=1e-2, grad_protection_threshold = 1e5, method='full', delta=0, strategy=np.array([1,2,3])):
        ...
        for i in tqdm(range(self.N)):
            if self.strat_1 and not strat1_fail:
                try:
                    ...
                    idoc_blocks = idoc_ineq.build_blocks_idoc(aux_sys, self.delta)
                    if self.method == "full":
                        traj_deriv = idoc_ineq.idoc_full(*idoc_blocks)
                        self.loss[i], grad = self.Traj_L2_Loss(traj, traj_deriv)
                    else:
                        pass
                        # TODO: ive done the reference traj differently
                        # self.loss[i] = self.Traj_L2_Loss(traj)
                        # grad = idoc_ineq.idoc_vjp(self.xr, traj, *idoc_blocks)
            ...
            if self.strat_2 and not strat2_fail:
                ...
                try:
                    ...
                    # get aux system
                    aux_sys_barrier1 = self.sys.barrier_oc.getAuxSys(
                        state_traj_opt=traj_barrier1['state_traj_opt'],
                        control_traj_opt=traj_barrier1['control_traj_opt'],
                        costate_traj_opt=traj_barrier1['costate_traj_opt'],
                        auxvar_value=traj_barrier1['auxvar_value']
                    )
                    
                    # get loss and gradient
                    idoc_blocks = idoc_eq.build_blocks_idoc(aux_sys_barrier1, self.delta)
                    if self.method == "full":
                        traj_deriv = idoc_eq.idoc_full(*idoc_blocks)
                        self.loss_barrier1[i], grad_barrier1 = self.Traj_L2_Loss(traj_barrier1, traj_deriv)
                    else:
                        pass
                        # TODO: ive done the reference traj differently
                        # self.loss_barrier1[i] = self.Traj_L2_Loss(traj_barrier1)
                        # grad_barrier1 = idoc_ineq.idoc_vjp(self.xr, traj_barrier1, *idoc_blocks)
            ...
            if self.strat_3 and not strat3_fail:
                try:
                    ...
                    # get aux sys 
                    aux_sys_barrier2 = self.sys.barrier_oc.getAuxSys(
                        state_traj_opt=traj_barrier2['state_traj_opt'],
                        control_traj_opt=traj_barrier2['control_traj_opt'],
                        costate_traj_opt=traj_barrier2['costate_traj_opt'],
                        auxvar_value=traj_barrier2['auxvar_value']
                    )

                    # get loss and gradient
                    idoc_blocks = idoc_eq.build_blocks_idoc(aux_sys_barrier2, self.delta)
                    if self.method == "full":
                        traj_deriv = idoc_eq.idoc_full(*idoc_blocks)
                        self.loss_barrier2[i], grad_barrier2 = self.Traj_L2_Loss(traj_barrier2, traj_deriv)
                    else:
                        pass
                        # TODO: ive done the reference traj differently
                        # self.loss_barrier2[i] = self.Traj_L2_Loss(traj_barrier2)
                        # grad_barrier2 = idoc_ineq.idoc_vjp(self.xr, traj_barrier2, *idoc_blocks)
    ...
    def Traj_L2_Loss(self, traj, aux_sol=None):
    ...
    def save_results(self, env, exp=None, true_X=None, plots=False, txt=False, npz=False):
        ...
        folder_name = "results/idoc/"+env
        if not os.path.exists(folder_name):
            os.makedirs(folder_name)

        if plots:
            folder_name2 = "results/idoc/"+env+str(exp or '')
        ...
        if txt:
            ...
            file.write(f"Method = "+self.method + f", delta = {self.delta:0.2e} \n")
            file.write("\n")
...
class IDOC_uncons:
    def __init__(self, OC_sys, reference_traj, init_params, learning_rate, num_iter, method='full', delta=0):
        ''' Reference Trajectory '''
        self.xr = reference_traj
        self.horizon = reference_traj.shape[0] - 1
    ...
    def run(self):
        ...
        for k in tqdm(range(self.N)):
            ...
            # get loss and gradient through idoc eq blocks
            idoc_blocks = idoc_eq.build_blocks_idoc(aux_sys, self.delta)
            if self.method == "full":
                traj_deriv = idoc_eq.idoc_full(*idoc_blocks)
                self.loss[k], grad = self.Traj_L2_Loss(traj, traj_deriv)
            else:
                pass
                # TODO: ive done the reference traj differently
                # self.loss_barrier1[i] = self.Traj_L2_Loss(traj_barrier1)
            ...
            # parameter update
            self.Xk[k+1] = self.Xk[k] - self.lr[k]*grad
