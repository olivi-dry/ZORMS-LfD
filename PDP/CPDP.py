...
class COCSys:
    ...
    def cocSolver(self, ini_state, horizon, auxvar_value=1, interplation_level=1, print_level=0):
        ...
        state_traj_opt = self.interpolation(time_grid, state_grid_opt, interplation_level)
        control_traj_opt = self.interpolation(time_grid, control_grid_opt, interplation_level)
        costate_traj_opt = self.interpolation(time_grid, costate_grid_opt, interplation_level)
    ...
    def auxSysSolver(self, opt_sol, auxvar_value=1):
        statement = [hasattr(self, 'dfx_fn'), hasattr(self, 'dfu_fn'), hasattr(self, 'dfe_fn'),
                     hasattr(self, 'ddHxx_fn'), hasattr(self, 'ddHxu_fn'), hasattr(self, 'ddHxe_fn'),
                     hasattr(self, 'ddHux_fn'),
                     hasattr(self, 'ddHuu_fn'), hasattr(self, 'ddHue_fn'), hasattr(self, 'ddhxx_fn'),
        ...
        # xulam = opt_sol(0)
        x= opt_sol['state_traj_opt'](0)
        u= opt_sol['control_traj_opt'](0)
        lam= opt_sol['costate_traj_opt'](0)
        vec_PW = vec_PW_sol(0)
        P = numpy.reshape(vec_PW[0:self.n_state * self.n_state], (self.n_state, self.n_state))
        W = numpy.reshape(vec_PW[self.n_state * self.n_state:], (self.n_state, -1))
        ...
        for k in range(0, self.n_grid):
            ...
            x= opt_sol['state_traj_opt'](time_grid[k + 1].item())
            u= opt_sol['control_traj_opt'](time_grid[k + 1].item())
            lam= opt_sol['costate_traj_opt'](time_grid[k + 1].item())
            vec_PW = vec_PW_sol(time_grid[k + 1].item())
            P = numpy.reshape(vec_PW[0:self.n_state * self.n_state], (self.n_state, self.n_state))
            W = numpy.reshape(vec_PW[self.n_state * self.n_state:], (self.n_state, -1))
