...
class COCsys:
    ...
    def ocSolver(self, horizon, ini_state=None, auxvar_value=1, print_level=0):
        ...
        if ini_state is None:
            ini_state = self.init_condition_fn(auxvar_value).full().flatten().tolist()
        else:
            ini_state = casadi.DM(ini_state).full().flatten().tolist()
    ...
    def solveBarrierOC(self, horizon, ini_state=None, auxvar_value=1):
        ...
        opt_sol = self.barrier_oc.ocSolver(ini_state=ini_state, horizon=horizon, auxvar_value=auxvar_value)
...
class EQCLQR:
    ...
    def eqctlqrSolver(self, ini_state=None, horizon=None, threshold=None):

        if ini_state is None:
            ini_state = self.init_state
        if horizon is None:
